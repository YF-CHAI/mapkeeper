...
public:
    ...
    void scanAscending(RecordListResponse& _return, const std::string& map,
              const std::string& startKey, const bool startKeyIncluded, 
              const std::string& endKey, const bool endKeyIncluded,
              const int32_t maxRecords, const int32_t maxBytes) {
        boost::shared_lock< boost::shared_mutex> readLock(mutex_);
        boost::ptr_map<std::string, leveldb::DB>::iterator itr = maps_.find(map);
        if (itr == maps_.end()) {
        ...
        Comparator* cmp = itr->second->user_comparator();
    ...
    void scanDescending(RecordListResponse& _return, const std::string& map,
              const std::string& startKey, const bool startKeyIncluded, 
              const std::string& endKey, const bool endKeyIncluded,
              const int32_t maxRecords, const int32_t maxBytes) {
        boost::shared_lock< boost::shared_mutex> readLock(mutex_);
